...
/** Calculate the centerline, tail, nose, midpoint of the fish **/
	...
	Rect boundRect = boundingRect(rough);
	RotatedRect box = minAreaRect(rough);
	Moments mu;
	Mat Dst_boundRect(Binary, boundRect);
	mu = moments(Dst_boundRect, false);
	Point2d weight_center = Point2d(mu.m10 / mu.m00 + boundRect.x, mu.m01 / mu.m00 + boundRect.y);
	//Mat rgbDst(Dst.size(), CV_8UC3);
	//cvtColor(Dst, rgbDst, COLOR_GRAY2BGR);
	//circle(rgbDst, weight_center, 6, Scalar(0, 255, 0));
	//circle(rgbDst, box.center, 3, Scalar(0, 0, 255));
	//imshow("Dst_Thresh", rgbDst);
	Point2d heading_vector_rough = Point2d(weight_center.x - box.center.x, weight_center.y - box.center.y);
