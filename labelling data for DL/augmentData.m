...
function augmentData(dataPath,result)
    sample_size = [320,320];
    ...
    path_augmented = [dataPath,'/augmented/'];
    if(~exist(path_augmented))
        mkdir(path_augmented);
    end
    path_rawdata=[dataPath,'/raw_data/'];
    allrawDataName=dir(fullfile(path_rawdata,'*.png'));
    extension='png';
    
    idx_sample = 1;
    labels = []; % [head_x, head_y, yolk_x, yolk_y, nonfish]
    for i = 1:length(allrawDataName)
        try
        filename_keyframe = [path_rawdata,allrawDataName(i).name];
        ...
        for k = 0:10:359
            ...
            filename_dst = [path_augmented,num2str(idx_sample,'%05d') , '.' , extension];
    ...
    % output the labels
    filename_dst = [path_augmented, 'labels.txt'];
